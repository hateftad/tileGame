...
void OGL::Program::SetUniform(const GLchar* name, const float scalar)
{
	glUniform1f(uniform(name), scalar);
}

void OGL::Program::SetUniform1f( const GLint ids, const float offset, GLboolean transpose /*= GL_FALSE*/ )
{
	assert(isInUse());
	glUniform1f(ids, offset);
}

void OGL::Program::bindAttrib(unsigned int index, const std::string& attribName)
{
	glBindAttribLocation(_object, index, attribName.c_str());
}
