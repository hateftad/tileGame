...
namespace OGL {
    ...
    public:
		...
		void linkProgram(){glLinkProgram(_object);}
...
\
		...
		void SetUniform1f(const GLint ids, const float offset, GLboolean transpose = GL_FALSE); 
		void bindAttrib(unsigned int index, const std::string& attribName);
        void SetUniform( const GLchar* name,  const float scalar );
    private:
        GLuint _object;
